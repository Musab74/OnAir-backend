...
export class RecordingUploadController {
  ...
  async uploadClientRecording(
    ...
    try {
      ...
      this.logger.log(
        `[CLIENT_RECORDING] Generated filename: ${finalFileName}`,
      );
      this.logger.log(
        `[CLIENT_RECORDING] Original file: ${file.originalname}, isWebM: ${isWebM}`,
      );
      ...
      if (isWebM) {
        try {
          ...
          this.logger.log(
            `[CLIENT_RECORDING] ✅ Converted WebM to MP4: ${finalFilePath}`,
          );
        } catch (conversionError) {
          this.logger.warn(
            `[CLIENT_RECORDING] ⚠️ WebM to MP4 conversion failed, using original: ${conversionError.message}`,
          );
      ...
      const vodUploadSuccess = await this.uploadToVodServer(
        finalFilePath,
        finalFileName,
      );
      ...
      if (!vodUploadSuccess) {
        throw new BadRequestException(
          'Failed to upload recording to VOD server',
        );
      }
      ...
      try {
        ...
        if (fs.existsSync(file.path)) {
          ...
          this.logger.log(
            `[CLIENT_RECORDING] Original file cleaned up: ${file.path}`,
          );
        }
        ...
        if (finalFilePath !== file.path && fs.existsSync(finalFilePath)) {
          ...
          this.logger.log(
            `[CLIENT_RECORDING] Converted file cleaned up: ${finalFilePath}`,
          );
        }
      } catch (cleanupError) {
        this.logger.warn(
          `[CLIENT_RECORDING] Warning: Could not clean up local file: ${cleanupError.message}`,
        );
      }
      ...
      this.logger.log(
        `[CLIENT_RECORDING] ✅ Recording uploaded successfully: ${recordingUrl}`,
      );
      this.logger.log(
        `[CLIENT_RECORDING] Final file size: ${finalFileSize} bytes (original: ${file.size} bytes)`,
      );
      ...
      try {
        ...
        await newVod.save();
        this.logger.log(
          `[CLIENT_RECORDING] ✅ VOD entry created: ${newVod._id} for meeting ${body.meetingId}`,
        );
      } catch (vodError) {
        this.logger.error(
          `[CLIENT_RECORDING] ❌ Failed to create VOD entry: ${vodError.message}`,
        );
      ...
      if (file?.path && fs.existsSync(file.path)) {
        try {
          ...
          this.logger.warn(
            `[CLIENT_RECORDING] Could not clean up original file on error: ${cleanupError.message}`,
          );
        }
      }
  ...
  private async convertWebMToMP4(
    ...
    this.logger.log(
      `[FFMPEG] Converting WebM to MP4: ${inputPath} -> ${outputPath}`,
    );
    ...
    try {
      ...
      if (!fs.existsSync(outputPath)) {
        throw new Error(
          'FFmpeg conversion completed but output file not found',
        );
      }
      ...
      this.logger.log(
        `[FFMPEG] ✅ Conversion complete. Input: ${inputStats.size} bytes, Output: ${outputStats.size} bytes`,
      );
  ...
  private async uploadToVodServer(
    ...
    try {
      ...
      const response = await axios.post(
        'https://i-vod1.hrdeedu.co.kr/upload',
        formData,
        {
          headers: {
            ...formData.getHeaders(),
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        this.logger.error(
          `[VOD_UPLOAD] ❌ VOD server returned status: ${response.status}`,
        );
        return false;
      }
    } catch (error) {
