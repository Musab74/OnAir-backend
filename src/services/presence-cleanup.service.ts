...
export class PresenceCleanupService {
  ...
  async cleanupStaleParticipants() {
    try {
      const cleanedCount =
        await this.participantService.cleanupStaleParticipants(150); // 150 seconds threshold - allows for heartbeat grace period

      if (cleanedCount > 0) {
      }
    } catch (error) {
      this.logger.error(
        `[CLEANUP] Error during stale participant cleanup: ${error.message}`,
      );
    }
  }
  ...
  async manualCleanup(thresholdSeconds: number = 90): Promise<number> {
    try {
      const cleanedCount =
        await this.participantService.cleanupStaleParticipants(
          thresholdSeconds,
        );
      return cleanedCount;
    } catch (error) {
      this.logger.error(
        `[MANUAL_CLEANUP] Error during manual cleanup: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async getStaleParticipantsStats(thresholdSeconds: number = 90): Promise<{
    ...
    try {
      ...
      const totalStale =
        await this.participantService.getStaleParticipantsCount(
          thresholdSeconds,
        );
      ...
      this.logger.error(
        `[STATS] Error getting stale participants stats: ${error.message}`,
      );
      return {
        totalStale: 0,
        staleInWaiting: 0,
