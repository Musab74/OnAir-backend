...
export class AppController {
  ...
  async getStaleParticipantsStats(
    @Query('thresholdSeconds') thresholdSeconds: string = '90',
  ) {
    const threshold = parseInt(thresholdSeconds, 10);
    return await this.presenceCleanupService.getStaleParticipantsStats(
      threshold,
    );
  }
  ...
  async manualCleanup(
    ...
    const threshold = parseInt(thresholdSeconds, 10);
    const cleanedCount =
      await this.presenceCleanupService.manualCleanup(threshold);
    return {
      success: true,
      cleanedCount,
